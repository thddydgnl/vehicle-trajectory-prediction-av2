...
$CompleteMarker = Join-Path $RunDir "FULL_PILOT_1EPOCH_COMPLETE.txt"
$FailedMarker = Join-Path $RunDir "FULL_PILOT_1EPOCH_FAILED.txt"
...
$Log = Join-Path $LogDir "full_pilot_1epoch_$Timestamp.log"
...
function Write-Status {
  ...
  $Artifacts = [ordered]@{
    ...
    lstm_best = Test-Artifact (Join-Path $RunDir "checkpoints\best_lstm_full_pilot_1epoch.pt")
    lstm_metrics = Test-Artifact (Join-Path $RunDir "metrics\lstm_full_pilot_1epoch_val_metrics.json")
    transformer_best = Test-Artifact (Join-Path $RunDir "checkpoints\best_transformer_full_pilot_1epoch.pt")
    transformer_metrics = Test-Artifact (Join-Path $RunDir "metrics\transformer_full_pilot_1epoch_val_metrics.json")
    pca_codec = Test-Artifact (Join-Path $RunDir "checkpoints\pca_codec.pkl")
    diffusion_pca_best = Test-Artifact (Join-Path $RunDir "checkpoints\best_diffusion_pca_full_pilot_1epoch.pt")
    diffusion_pca_metrics = Test-Artifact (Join-Path $RunDir "metrics\diffusion_pca_full_pilot_1epoch_val_metrics.json")
    diffusion_direct_best = Test-Artifact (Join-Path $RunDir "checkpoints\best_diffusion_direct_full_pilot_1epoch.pt")
    diffusion_direct_metrics = Test-Artifact (Join-Path $RunDir "metrics\diffusion_direct_full_pilot_1epoch_val_metrics.json")
...
try {
  Write-Status -Status "running" -Step "preflight" -Message "Full AV2 1-epoch pilot started"
  Add-Content -Path $Log -Value "[$(Get-Date -Format o)] Starting Full AV2 1-epoch pilot"
  ...
  Invoke-CondaPython "linear_evaluation" @(
    ...
    "--prediction_tag", "full_pilot_1epoch"
  )

  Invoke-CondaPython "lstm_training" @(
    "-m", "src.training.train",
    "--config", "configs/full_pilot_lstm_1epoch.yaml",
    "--data", $TrainData,
    "--val_data", $ValData
  )
  ...
  Invoke-CondaPython "transformer_training" @(
    "-m", "src.training.train",
    "--config", "configs/full_pilot_transformer_1epoch.yaml",
    "--data", $TrainData,
    "--val_data", $ValData
  )
  ...
  Invoke-CondaPython "diffusion_pca_training" @(
    "-m", "src.training.train",
    "--config", "configs/full_pilot_diffusion_pca_1epoch.yaml",
    "--data", $TrainData,
    "--val_data", $ValData
  )
  ...
  if (-not $SkipDirectDiffusion) {
    Invoke-CondaPython "diffusion_direct_training" @(
      "-m", "src.training.train",
      "--config", "configs/full_pilot_diffusion_direct_1epoch.yaml",
      "--data", $TrainData,
      "--val_data", $ValData
    )
  }
  ...
  $FinalEvalArgs = @(
    ...
    "--checkpoint_tag", "full_pilot_1epoch",
    "--batch_size", "64",
    "--data_split", "val_full",
    "--target_type", "av2_focal_mixed",
    "--prediction_tag", "full_pilot_1epoch"
  ...
  Set-Content -Path $CompleteMarker -Value "Completed Full AV2 1-epoch pilot at $(Get-Date -Format o). Log: $Log" -Encoding UTF8
  Write-Status -Status "complete" -Step "complete" -ExitCode 0 -Message "Full AV2 1-epoch pilot complete"
  exit 0
} catch {
  $Message = $_.Exception.Message
  Add-Content -Path $Log -Value "[$(Get-Date -Format o)] FAILED: $Message"
  Set-Content -Path $FailedMarker -Value "Failed Full AV2 1-epoch pilot at $(Get-Date -Format o). Log: $Log. Error: $Message" -Encoding UTF8
